...
int mosaik2_gathering(mosaik2_arguments *args) {
	...
	if(thumbs_tile_count*thumbs_tile_count*(6*256)>UINT32_MAX) {
		//can candidates_cost contain the badest possible cost?
...
//	printf("analyze master image\n");
  ...
  //double total_pixel_per_primary_tile = pixel_per_primary_tile * pixel_per_primary_tile;
	...
	*/
	...
	unsigned long long *candidates_cost = malloc(total_candidates_count * sizeof(unsigned long long));
	if( candidates_cost == NULL ) { fprintf(stderr, "could not allocate memory for candidates_cost\n"); exit(EXIT_FAILURE); }
...
//	uint64_t thumbs_db_element_count = st.st_size/2;
	...
	while(1) {
		...
		for(uint32_t i=0,len1=len-1;i<len1;i+=2,idx++) {
			...
			//uint32_t candidates_cost_f = UINT32_MAX;
...
/*								if(diff_color0>=176093773879||diff_color0<0) {
								...
								//if(diff_color0 < candidates_cost[primary_tile_idx] ) {
								                 // badest cost is saved in the last position, and this position 
																 // will expand from 0 to total_primary_tile_count
								uint32_t offset = primary_tile_idx * max_candidates_len;
								unsigned long long old_cost=ULLONG_MAX;

								

								if(candidates_len[primary_tile_idx]>0 && candidates_len[primary_tile_idx] >= max_candidates_len)
									old_cost = candidates_cost[offset + candidates_len[primary_tile_idx] -1];

								if(diff_color0 < old_cost) {

									
									
									if(debug)
										printf("idx:%lu primary_tile_idx:%05i diff_color0:%05llu,old_cost:%05llu,len:%i %i\n",idx,primary_tile_idx,diff_color0,old_cost,candidates_len[primary_tile_idx], max_candidates_len);
									if(unique==1) {
										int continue_=0;
										//if there is the same candidate already with a lower cost in it
										...
										for(uint32_t i=0;i<candidates_len[primary_tile_idx];i++) {
											if(candidates_index[offset + i] == idx) {
												...
												if( candidates_cost[offset + i] <= diff_color0) {
													if(debug)fprintf(stdout, "ignore new candidate new_cost:%llu, old_cost:%llu\n",diff_color0,candidates_cost[offset+i]);
													continue_=1;
													break;
												} else {
													if(debug)
													...
													for(uint32_t j=i;j<candidates_len[primary_tile_idx]-1;j++) {
														...
														candidates_cost[offset+j] = candidates_cost[offset+j+1];
									...
									for(; insert_pos < candidates_len[primary_tile_idx]; insert_pos++) {
										if( diff_color0 < candidates_cost[offset + insert_pos] )
									...
									//position to insert new cost found
									...
									for(int i=candidates_len[primary_tile_idx] - 1;i>insert_pos;i--) {
										...
										candidates_cost[offset + i] = candidates_cost[offset + i - 1];
									...
									candidates_cost[offset + insert_pos] = diff_color0;
									...
									fprintf(stdout, "\ncandidates_cost[%i:%i-%i]:{", primary_tile_idx, 0, max_candidates_len);
//									fprintf(stdout, "%05lli ", candidates_cost[primary_tile_idx + insert_pos ], diff_color0);
									for( int i=0;i<max_candidates_len;i++) {
										if(candidates_len[primary_tile_idx]==i)
											fprintf(stdout, "}");
										fprintf(stdout, "%llu,", candidates_cost[offset + i]);
									}
										...
										//TODO dont know why, somewhere in this program a routine seems to change the offsets. Can anybody identify this error source?
											if(candidates_off_x[i*max_candidates_len] != 0 && candidates_off_y[i*max_candidates_len] != 0 ) {
												...
												fprintf(stderr, "%i	%li	%llu	%i	%i\n",
													i,
													candidates_index[i*max_candidates_len],
													candidates_cost[i*max_candidates_len],
													candidates_off_x[i*max_candidates_len],
													candidates_off_y[i*max_candidates_len]);
	...
	free( stddev_blue_int );
									...
									fprintf(stdout, "\ncandidates_cost[%i]:{", primary_tile_idx);
//									fprintf(stdout, "%05lli ", candidates_cost[primary_tile_idx + insert_pos ], diff_color0);
									for( int i=0;i<max_candidates_len;i++) {
										if(candidates_len[primary_tile_idx]==i)
											fprintf(stdout, "}");
		...
		do {
			...
			for(uint32_t i = 0;i<total_primary_tile_count-1;i++) {
				for(uint32_t j = 0;j<total_primary_tile_count;j++) {
					...
					if(debug)fprintf(stdout, "i:%i:%i %i j:%i:%i %i idx:%lu %lu cost:%llu %llu off:%i,%i %i,%i\n", 
						i,i*max_candidates_len,offset_i,                        j, j*max_candidates_len,offset_j,
						
						candidates_index[offset_i], candidates_index[offset_j],
						candidates_cost[offset_i], candidates_cost[offset_j], 
					...
					if( candidates_index[offset_i] == candidates_index[offset_j] ) {
						if( candidates_cost[offset_i] >= candidates_cost[offset_j]) 
							candidates_elect[i]++;
						else
							candidates_elect[j]++;
...
if(debug) {
	...
	// total_primary_tile_count * max_candidates_len
		...
		fprintf(mosaik2_result,"%i	%li	%llu	%i	%i\n",
				...
				candidates_cost[offset],candidates_off_x[offset],candidates_off_y[offset]);
	}
	fclose(mosaik2_result);
