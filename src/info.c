...
void print_element(mosaik2_arguments *args, char* mosaik2_db_name, mosaik2_database *md, int element_number) {
	...
	read_entry(md->tileoffsets_filename, &val_char, sizeof(char)*2, element_number*2*sizeof(char));
	if(val_char[0] == 0xFF && val_char[1] == 0xFF) {
		printf("tileoffsets=unset\n");
	} else if( val_char[0] != 0 && val_char[1] != 0) {
		printf("tileoffsets=invalid\n");
	} else {
		printf("tileoffsets=%ix%i\n", val_char[0], val_char[1]);
	}
}
